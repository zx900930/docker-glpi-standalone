...
docker run --name glpi \
  ...
  -p 8080:80 \
  -d triatk/glpi-standalone:11.0.8
```
...
Le tag de l'image Docker correspond souvent à une version de GLPI (ex: `triatk/glpi-standalone:11.0.8`).
...
services:
  ...
  glpi:
    ...
    image: triatk/glpi-standalone:11.0.8 # Utilisez un tag de version spécifique
    ...
    environment:
      ...
      # - VERSION_GLPI=11.0.8 # Souvent déterminé par le tag de l'image, mais peut être défini
    restart: unless-stopped
```
...
services:
  ...
  glpi:
    ...
    environment:
      ...
      # - VERSION_GLPI=11.0.8 # Généralement défini par le tag de l'image, confirmez si besoin
...
- **Note :** Préférez utiliser un tag d'image spécifique comme `triatk/glpi-standalone:11.0.8`.
