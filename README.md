...
The Docker image tag often corresponds to a GLPI version (e.g., `triatk/glpi-standalone:11.0.7`).
...
services:
  ...
  glpi:
    ...
    image: triatk/glpi-standalone:11.0.7 # Use a specific version tag
    ...
    environment:
      ...
      # - VERSION_GLPI=11.0.7 # Often determined by the image tag, but can be set
    restart: unless-stopped
```
...
services:
  ...
  glpi:
    ...
    environment:
      ...
      # - VERSION_GLPI=11.0.7 # Usually set by the image tag, confirm if needed
...
- **Note:** Prefer using a specific image tag like `triatk/glpi-standalone:11.0.7`.
